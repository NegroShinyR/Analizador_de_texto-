...
def tokens_significativos(toks):
...
# =========================
# Vocabulario desde Postgres
# =========================
@st.cache_data
def load_vocab_from_postgres() -> pd.DataFrame:
    """
    Carga el vocabulario desde la tabla public.palabras_clave
    Campos: palabra TEXT, porcentaje_identidad NUMERIC, sinonimos TEXT[]
    """
    import psycopg2
    pg = st.secrets["pg"]
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT palabra, porcentaje_identidad, COALESCE(sinonimos, '{}') AS sinonimos
                FROM public.palabras_clave
            """)
            rows = cur.fetchall()
    finally:
        conn.close()
    ...
    # Asegura lista de sinónimos; filtra vacíos y espacios
    def clean_syn(arr):
        out = []
        if arr is None:
            return out
...
def build_vocab_index(vocab_df: pd.DataFrame) -> Dict[str, Tuple[str, float]]:
    ...
    for _, row in vocab_df.iterrows():
        base = row["palabra"]          # como está en la BD
        w = float(row["peso"])
        ...
        for s in row["sinonimos"]:
            ...
            if s_key:                  # evita cadenas vacías
                index[s_key] = (base, w)
    return index


# =========================
# Algoritmo MatCompat v5 (estabilidad mejorada)
# =========================
def matcompat_score(text: str, vocab_index: Dict[str, Tuple[str, float]]):
    ...
    Esto evita saturación y garantiza:
      - Textos claramente matemáticos → altos
      - Textos mixtos → medios
      - Textos no matemáticos → bajos
    """
...
def app():
    ...
    # 1) Cargar vocab de Postgres
    with st.spinner("Cargando vocabulario de Matemáticas…"):
        vocab_df = load_vocab_from_postgres()
        vocab_idx = build_vocab_index(vocab_df)
    st.success(f"Vocabulario cargado: {len(vocab_df):,} términos base.")
